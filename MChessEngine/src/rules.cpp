...
namespace std {
	...
	position* position::make_move( _move m ) {
		...
		if( WKS_CASTLE <= modifier && modifier <= BQS_CASTLE ) {
			...
			if( actor == null_piece ) {
				cout << "tried to move nonexistent piece at " << std::hex << ( int )initialLocation << endl;
				return this;
			}

			std::cout << "piece's current position: " << std::hex << setw( 2 ) << setfill( '0' ) << ( actor & 255 ) << endl;

			_piece& enPassantPawn = piece_search( details & 0x000ff000 );
			...
			default:
				...
				if( modifier > 6 ) {
					switch( modifier ) {
					case 4 + KNIGHT:
						actor &= ( sizeof( unsigned short ) - 1792 ) & ( KNIGHT << 8 );
						break;
					case 4+BISHOP:
						actor &= ( sizeof( unsigned short ) - 1792 ) & ( BISHOP << 8 );
						break;
					case 4+ROOK:
						actor &= ( sizeof( unsigned short ) - 1792 ) & ( ROOK << 8 );
						break;
					case 4+QUEEN:
						actor &= ( sizeof( unsigned short ) - 1792 ) & ( QUEEN << 8 );
						break;
					}
				}
				//Plain old move, possibly with a capture
				else {
					_piece& potentialVictim = piece_search( finalLocation );

					if( potentialVictim != null_piece ) {
						std::cout << "potential victim piece: " << hex << setfill( '0' ) << setw( 4 ) << potentialVictim << endl;
						std::cout << "if killed, we should get simply another piece: " << hex << setfill( '0' ) << setw( 4 ) << kill( potentialVictim ) << endl;
					}
			...
			std::cout << "moved from " << std::hex << ( short )initialLocation
			          << " to " << std::hex << ( short ) finalLocation << endl;
		...
		//no more en passant for you!
		//but only if en passant wasn't set just this round...
