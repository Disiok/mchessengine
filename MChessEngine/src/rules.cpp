...
namespace myriad {
	...
	void position::make_move(_move m) {
		...
		case 0:
			...
			if((type = get_piece_type(moving)) == ROOK) {
				switch(start) {
				case 0x00:
					details = revoke_castle_right(details, WQS_CASTLE);
					break;
				case 0x07:
					details = revoke_castle_right(details, WKS_CASTLE);
					break;
				case 0x70:
					details = revoke_castle_right(details, BQS_CASTLE);
					break;
				case 0x77:
					details = revoke_castle_right(details, BKS_CASTLE);
					break;
				}
			}
		...
		if(threats_size != 0) {
			...
			if(threats_size >= 2) return moves;	/* Only moving the king is possible on double check. */
				...
				if(threat_type != PAWN && threat_type != KNIGHT) {
					const char diff = getDifference(threat_loc, c_loc);
	...
	void position::unmake_move(_move previous_move, _property prev_details) {
		...
		case 0:
		case DOUBLE_ADVANCE:
			moved = move_piece(moved, end, start);
			break;
		case WKS_CASTLE:
		case BKS_CASTLE:
			moved += 0x02;
			...
			while((current & 0x88) == 0) {
				...
				if(obstruct != 0) {
					...
					case ROOK  :
						...
						if(i < 4) to_return.push_back(obstruct);

						break;
					case BISHOP:

						if(i > 3) to_return.push_back(obstruct);

						break;
					case QUEEN :
						to_return.push_back(obstruct);
						break;
					}
					...
					if(!guardian_found) {
						...
						if(get_piece_color(obstruct) == col) break;
							...
							case ROOK:
								...
								if(i < 4) attacker = obstruct;

								set = true;
								break;
							case BISHOP:

								if(i > 3) attacker = obstruct;
